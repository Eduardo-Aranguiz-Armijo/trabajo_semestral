...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorResponse> handleWebClient(
        ...
        if (ex instanceof WebClientResponseException) {
            details = ((WebClientResponseException) ex).getResponseBodyAsString();
        }
        return ResponseEntity
                .status(HttpStatus.BAD_GATEWAY)
                .body(ErrorResponse.of(
                        HttpStatus.BAD_GATEWAY,
                        "Error detallado de customer-service: " + details,
                        request.getRequestURI()
                ));
    }
    ...
    public ResponseEntity<ErrorResponse> handleIllegalState(
        ...
        log.error("IllegalStateException en auth-service: {}", ex.getMessage(), ex);
        ...
        if (ex.getMessage() != null && ex.getMessage().contains("No instances available")) {
            return ResponseEntity
                    ...
                    .body(ErrorResponse.of(
                            ...
                            "Servicio no disponible temporalmente. Esperando sincronización de Eureka.",
                            request.getRequestURI()
                    ));
        }
        
        return handleRuntime(ex, request);
    }
